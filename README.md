...
Sveltia CMS at `/admin`, Pagefind search, Giscus comments, deployed to **Cloudflare
Pages** by a GitHub Actions workflow that runs Wrangler.

**Planned live URL:** <https://cloudflare-blog.pages.dev/> — not deployed yet; see
[Status](#status) below.
...
| --- | --- |
| [docs/setup.md](docs/setup.md) | **Start here.** One-time setup — GitHub repo, Cloudflare Pages project, deploy secrets, CMS token, Giscus, contact form, author details |
...
| `npm run deploy` | One-off local deploy: `wrangler pages deploy dist` — CI does this on every push |
...
```bash
grep -rhoE 'https?://[^"< ]+' dist --include=*.html --include=*.xml   | grep -v 'cloudflare-blog.pages.dev' | sort -u
```

## Status

This project was scaffolded from the sibling GitHub Pages blog and has **not been
deployed yet**. Nothing below is done:

- GitHub repository created and pushed
- `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` set as Actions secrets
- Cloudflare Pages project `cloudflare-blog` created
- First deploy verified live
- CMS access token, Giscus IDs, contact form endpoint, author details in `src/consts.ts`

The site builds and runs locally (`npm run dev` / `npm run build` / `npm run preview`)
independent of all of the above. Full checklist in [docs/setup.md](docs/setup.md).
